...
class UNetResnet(base_model.BaseModel):
    def __init__(self, num_classes, in_channels=3, backbone='resnet50', pretrained=True, freeze_bn=False, freeze_backbone=False, **_):
        ...
        #get the number of features of the each layer of the encoder
        if isinstance(model.layer1[-1], BasicBlock):
            #batch norm is the final layer of the block
            num_features_layer1 = list(model.layer1[-1].modules())[-1].num_features
        elif isinstance(model.layer1[-1], Bottleneck):
            #batch norm is the second to last layer of the block (last is the ReLU)
            num_features_layer1 = list(model.layer1[-1].modules())[-2].num_features
        
        if isinstance(model.layer2[-1], BasicBlock):
            num_features_layer2 = list(model.layer2[-1].modules())[-1].num_features
        elif isinstance(model.layer2[-1], Bottleneck):    
            num_features_layer2 = list(model.layer2[-1].modules())[-2].num_features
            
        if isinstance(model.layer3[-1], BasicBlock):
            num_features_layer3 = list(model.layer3[-1].modules())[-1].num_features
        elif isinstance(model.layer3[-1], Bottleneck):    
            num_features_layer3 = list(model.layer3[-1].modules())[-2].num_features
            
        if isinstance(model.layer4[-1], BasicBlock):
            num_features_layer4 = list(model.layer4[-1].modules())[-1].num_features 
        elif isinstance(model.layer4[-1], Bottleneck):    
            num_features_layer4 = list(model.layer4[-1].modules())[-2].num_features
        # decoder
        #numer of channels should be output of channels of the encoder + the number of channels of the previous layer
        self.conv1 = nn.Conv2d(num_features_layer4, 192, kernel_size=3, stride=1, padding=1)
        self.upconv1 =  nn.ConvTranspose2d(192, 128, 4, 2, 1, bias=False)

        self.conv2 = nn.Conv2d(num_features_layer3+128, 128, kernel_size=3, stride=1, padding=1)
        self.upconv2 = nn.ConvTranspose2d(128, 96, 4, 2, 1, bias=False)

        self.conv3 = nn.Conv2d(num_features_layer2+96, 96, kernel_size=3, stride=1, padding=1)
        self.upconv3 = nn.ConvTranspose2d(96, 64, 4, 2, 1, bias=False)

        self.conv4 = nn.Conv2d(num_features_layer1+64, 64, kernel_size=3, stride=1, padding=1)
