from . import base_dataloader, base_dataset
...
class CityScapesDataset(base_dataset.BaseDataSet):
...
class CityScapes(base_dataloader.BaseDataLoader):
...
from . import base_dataset, base_dataloader
from utils import palette
from glob import glob
import numpy as np
import os
...
class CityScapes(base_dataloader.BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1, mode='fine', val=False,
                    shuffle=False, transform=None, val_split= None, return_id=False):
